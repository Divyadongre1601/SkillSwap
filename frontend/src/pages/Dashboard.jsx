...
import {
    ...
    Select, Chip, Avatar, Card, CardContent, Rating, Button, Skeleton
} from '@mui/material';
...
function MatchBadge({ score }) {
    ...
    return (
        <span style={{
            ...
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: text, display: 'inline-block', flexShrink: 0 }} />
            <span style={{ fontSize: 11, fontWeight: 700, color: text, lineHeight: 1 }}>
                {score}% match
            </span>
        </span>
    );
    ...
    return (
        <div
            ...
            style={{
                ...
                height: '100%',          /* fills the fixed-height grid cell */
        ...
        >
            ...
            <div style={{ marginBottom: 10, flexShrink: 0 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.06em', marginBottom: 5 }}>
                    TEACHES
                </div>
...
export default function Dashboard() {
    ...
    const filtered = filter === 'all'
        ? users
        : users.filter(u => u.skillsOffered.some(s => s.toLowerCase() === filter.toLowerCase()));

    const sorted = [...filtered].sort((a, b) =>
        (scores[b._id.toString()] ?? -1) - (scores[a._id.toString()] ?? -1)
    );
            ...
            <Container maxWidth="xl" sx={{ py: 4, px: { xs: 2, md: 4 } }}>
                ...
                <Box sx={{ mb: 4 }}>
                    <Typography variant="h4" fontWeight={800}>
                        Welcome back, {loggedIn.name?.split(' ')[0] || 'there'} 👋
                    </Typography>
                    <Typography color="text.secondary" mt={0.5} fontSize={14}>
                        Matched users appear first — update your skills to improve matches
                    </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 4, flexWrap: 'wrap' }}>
                    {[
                        ...
                        ['Your matches', Object.keys(scores).length],
                        ['Showing',      sorted.length],
                    ].map(([label, val]) => (
                        <Box key={label} sx={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 2, px: 2.5, py: 1.5, minWidth: 100 }}>
                            <Typography fontSize={22} fontWeight={800} color="primary">{val}</Typography>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                    <Box sx={{ flex: 1, minWidth: 240, position: 'relative' }}>
                        <SearchIcon sx={{
                            ...
                            transform: 'translateY(-50%)', color: '#94A3B8', fontSize: 18, zIndex: 1,
                        }} />
                        <input
                ...
                {/* ── CSS Grid — every cell is exactly 310px tall ── */}
                {loading ? (
                    <div style={{
                        display: 'grid',
                        ...
                        {[...Array(6)].map((_, i) => (
                            <Skeleton key={i} variant="rounded" height={310} sx={{ borderRadius: 3 }} />
                        ))}
                    </div>
                ) : sorted.length === 0 ? (
                    ...
                    <div style={{
                        display: 'grid',
                        gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))',
                        gridAutoRows: '310px',   /* ← every row is exactly 310px, no exceptions */
                        gap: 20,
                    }}>
                        {sorted.map(u => (
                            <UserCard key={u._id} user={u} matchScore={scores[u._id.toString()]} />
                        ))}
                    </div>
                )}
